...
object FilterUtils:
  def someSubtreeMatches(tree: Tree, filter: Filter)(using Context): Boolean =
    tree.walkTree(filter.matches)((x, y) => y || x, false)
  def bodyThrows[E <: Exception](body: => Unit): Boolean =
...
def currentFilter: Filter = Filter({
  case tree if FilterUtils.someSubtreeMatches(tree, Filter({
  ...
  case ValDef(_, _, _, symbol) if FilterUtils.bodyThrows[UnsupportedOperationException](symbol.signature) =>  //Erasure: AndType
  case DefDef(_, _, _, _, symbol) if FilterUtils.bodyThrows[UnsupportedOperationException](symbol.signature) =>  //Erasure: AndType
